        ...
        <div class="mb-8" v-if="prefixes.length > 0">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <div v-for="it in prefixes" :key="String(it)"
                    ...
                    <div @click="changeFolder(String(it))" class="flex items-center gap-3">
                        <div
                    ...
                    <button v-if="it !== '/'" @click.stop="handleDeleteFolder(String(it))"
                        class="ml-auto p-1 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all"
...
const handleDeleteFolder = (folderPath: string) => {
    ElMessageBox.confirm(
        t('manage.deleteFolderConfirm'),
        t('manage.deleteFolder'),
        {
        ...
        try {
            await deleteFolder({ name: folderPath })
            ElMessage.success(t('manage.folderDeleted'))
            listImages()
        } catch (err) {
            console.error(err)
            ElMessage.error(t('manage.folderDeleteFailed'))
        }
    }).catch(() => {
        // user cancelled
    })
