...
imageRoutes.post('/list', listRateLimit, auth, async (c) => {
    ...
    try {
        ...
        // 获取子文件夹列表（如果不是搜索模式）
        // UNION 合并 images.folder 列和 folders 表，确保空目录也能显示
        let prefixes: string[] = []
        if (!keyword) {
            const folderQuery = viewAll
                ? `SELECT DISTINCT folder FROM images
                   ...
                   UNION
                   SELECT DISTINCT path AS folder FROM folders
                   WHERE path LIKE ? AND path != ?`
                : `SELECT DISTINCT folder FROM images
                   WHERE user_login = ? AND folder LIKE ? AND folder != ? AND folder IS NOT NULL
                   UNION
                   SELECT DISTINCT path AS folder FROM folders
                   WHERE user_login = ? AND path LIKE ? AND path != ?`

            const folderParams = viewAll
                ? [`${folder}%`, folder, `${folder}%`, folder]
                : [user!.login, `${folder}%`, folder, user!.login, `${folder}%`, folder]
